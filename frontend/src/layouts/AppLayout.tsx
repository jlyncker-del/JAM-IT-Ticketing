...
export function isSidebarLinkActive(to: string, pathname: string, search: string): boolean {
  const parameters = new URLSearchParams(search);
  if (to === "/tickets?unassigned=true") return pathname === "/tickets" && parameters.get("unassigned") === "true";
  if (to === "/tickets/neu") return pathname === "/tickets/neu";
  if (to === "/tickets") return (pathname === "/tickets" && parameters.get("unassigned") !== "true") || (pathname.startsWith("/tickets/") && pathname !== "/tickets/neu");
  return pathname === to;
}
...
export function AppLayout() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? "open" : ""}`} aria-label="Hauptnavigation">
      ...
      <nav className="sidebar-nav">{links.map(({ to, label, icon: Icon }) => { const active = isSidebarLinkActive(to, location.pathname, location.search); return <Link key={`${to}-${label}`} to={to} onClick={() => setOpen(false)} className="nav-link" aria-current={active ? "page" : undefined}><Icon size={19} aria-hidden="true" /><span>{label}</span>{active && <span className="sr-only">Aktuelle Seite</span>}</Link>; })}</nav>
    ...
    <div className="app-content">
      <header className="header"><div className="header-left"><button className="menu-button" aria-label={open ? "Navigation schließen" : "Navigation öffnen"} aria-expanded={open} onClick={() => setOpen((value) => !value)}>{open ? <X /> : <Menu />}</button><div className="header-title"><strong>{appConfig.appName}</strong><span>Support- und Ticketmanagement</span></div></div><div className="header-right"><Link to="/benachrichtigungen" className="icon-button" aria-label={`Benachrichtigungen, ${notifications.data?.filter((item) => !item.readAt).length ?? 0} ungelesen`}><Bell size={19} />{Boolean(notifications.data?.some((item) => !item.readAt)) && <span className="notification-count">{Math.min(99, notifications.data!.filter((item) => !item.readAt).length)}</span>}</Link><div className="user-summary"><span className="avatar" aria-hidden="true">{initials}</span><div><strong>{user.firstName} {user.lastName}</strong><span>{roleLabels[user.role]}</span></div></div></div></header>
